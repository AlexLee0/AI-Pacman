...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return not pycoSAT(to_cnf(premise & ~conclusion))
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    expression = []
    for literal1, literal2 in itertools.combinations(literals, 2):
        expression.append(disjoin(~literal1, ~literal2))
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for coord in all_coords:
        pacphysics_sentences.append(PropSymbolExpr(
            wall_str, coord[0], coord[1]) >> ~PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))

    possibleCoordsNonOuterWall = []
    for non_outer_wall_coord in non_outer_wall_coords:
        possibleCoordsNonOuterWall.append(PropSymbolExpr(
            pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t))
    pacphysics_sentences.append(exactlyOne(possibleCoordsNonOuterWall))

    possibleActions = []
    for direction in DIRECTIONS:
        possibleActions.append(PropSymbolExpr(direction, time=t))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        possibleActions = []
        print(t)
        for non_wall_coord in non_wall_coords:
            possibleCoordsNonOuterWall.append(PropSymbolExpr(
                pacman_str, non_wall_coord[0], non_wall_coord[1], time=t))
        for direction in DIRECTIONS:
            possibleActions.append(PropSymbolExpr(direction, time=t))
        KB.append(exactlyOne(possibleCoordsNonOuterWall))
        KB.append(exactlyOne(possibleActions))

        if t > 0:
            for non_wall_coord in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(
                    non_wall_coord[0], non_wall_coord[1], t, walls_grid))
...
def foodLogicPlan(problem) -> List:
    ...
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    for dot in food:
        KB.append(PropSymbolExpr(food_str, dot[0], dot[1], time=0))
    ...
    for t in range(50):
        ...
        print(t)
        for dot in food:
            goalAssertion.append(~PropSymbolExpr(
                food_str, dot[0], dot[1], time=t))
        for non_wall_coord in non_wall_coords:
            possibleCoordsNonOuterWall.append(PropSymbolExpr(
                pacman_str, non_wall_coord[0], non_wall_coord[1], time=t))
        ...
        if t > 0:
            for dot in food:
                KB.append(PropSymbolExpr(
                    food_str, dot[0], dot[1], time=t) % conjoin([PropSymbolExpr(food_str, dot[0], dot[1], time=t - 1), ~PropSymbolExpr(pacman_str, dot[0], dot[1], time=t - 1)]))
            for non_wall_coord in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(
                    non_wall_coord[0], non_wall_coord[1], t, walls))

        KB.append(conjoin(goalAssertion))
        model = findModel(conjoin(KB))
        KB.pop()
...
def localization(problem, agent) -> Generator:
    ...
    for non_outer_wall_coord in non_outer_wall_coords:
        if non_outer_wall_coord not in walls_list:
            KB.append(~PropSymbolExpr(wall_str,
                      non_outer_wall_coord[0], non_outer_wall_coord[1]))
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for non_outer_wall_coord in non_outer_wall_coords:
            if findModel(conjoin(conjoin(KB), PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t))):
                possible_locations.append(non_outer_wall_coord)
            if entails(conjoin(KB), PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t)):
                KB.append(PropSymbolExpr(
                    pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t))
            if entails(conjoin(KB), ~PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t)):
                KB.append(~PropSymbolExpr(
...
def mapping(problem, agent) -> Generator:
    ...
    KB.append(fourBitPerceptRules(0, agent.getPercepts()))
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords,
        ...
        for non_outer_wall_coord in non_outer_wall_coords:
            if entails(conjoin(KB), PropSymbolExpr(wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1])):
                KB.append(PropSymbolExpr(
                    wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1]))
                known_map[non_outer_wall_coord[0]
                          ][non_outer_wall_coord[1]] = 1
            if entails(conjoin(KB), ~PropSymbolExpr(wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1])):
                KB.append(~PropSymbolExpr(
                    wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1]))
                known_map[non_outer_wall_coord[0]
                          ][non_outer_wall_coord[1]] = 0
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.append(PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0))
    KB.append(numAdjWallsPerceptRules(0, agent.getPercepts()))
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords,
