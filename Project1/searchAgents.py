...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # must keep track of visited corners in each state in order to know what corners are left to complete goal
        visitedCorners = []
    ...
    def isGoalState(self, state: Any):
        ...
        """
        # visiting 4 corners means all corners have been visited
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            nextState = (nextx, nexty)
            # ensure next state is not wall
            if not hitsWall:
                # if next state is unvisited corner
                if nextState in self.corners and nextState not in state[1]:
                    # the corner will be visited when next state is taken, so we must include previously unvisited corner to state
                    successors.append(
                        ((nextState, state[1] + [nextState]), action, 1))
                else:
                    # the next state is not corner, we do not need to change our visited corners list
                    successors.append(
                        ((nextState, state[1]), action, 1))
        self._expanded += 1  # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    heuristic = 0
    # do not stop calculating heuristic until goal is complete
    while len(unvisitedCorners) != 0:
        # find most optimal corner to go to first by iterating through each unvisited corner and going to corner with lowest heuristic
        potentialHeuristics = {}
        for unvisitedCorner in unvisitedCorners:
            potentialHeuristics[unvisitedCorner] = util.manhattanDistance(
                currentState, unvisitedCorner)
        currentState = min(potentialHeuristics, key=potentialHeuristics.get)
        # assuming pacman goes to this corner, we must remove this corner from unvisited list, add to final heuristic, then repeat with remaining unvisited corners
        unvisitedCorners.remove(currentState)
        heuristic += potentialHeuristics[currentState]
    return heuristic
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodGrid.asList():
        # track distance from position to food
        potentialHeuristics.append(
            (mazeDistance(position, food, problem.startingGameState)))
    # if no more food
    if len(potentialHeuristics) == 0:
        return 0
    # limit states explored by returning maximum heuristic
    return max(potentialHeuristics)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # bfs will find closest food since it explores outward from starting state
